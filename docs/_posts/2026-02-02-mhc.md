...
$$
x_{L} = \left(\prod_{i=1}^{L-l} \mathcal{H}_{L-i}^\text{res}\right) x_l + \sum_{i=l}^{L-1}\left(\prod_{j=1}^{L-1-i} H_{L-j}^{res}\right)H_{i}^{\text{post}\top} \mathcal{F}(\mathcal{H}_i^{\text{pre}} x_i, \mathcal{W}_i)
$$
