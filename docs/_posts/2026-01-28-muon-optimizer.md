...
AdamW （/ˈædəm ˈdʌbəljuː/）的全称是 `Adam with Weight Decay` 缩写。
...
其实自己的思路可能要变一变，如何把它快速的接入到 [miniMind](https://github.com/jingyaogong/minimind) 项目中。

首先我们要知道 miniMind 是什么？

我理解一下，就是说，这个仓库是说，它可以让用户自己训练一个模型，来体验和了解模型训练的全过程。有点像是一个教学的模型，类似于 minix 之于 linux。

但是我其实对 pretrain/SFT/强化学习 这 3 步分别干了什么，也不清楚。

我们先看当前的这个仓库的 optimizer 是什么，根据和 Claude Code 对话，已经发现当前的 optimizer 是 AdamW：

在文件 `train_pretrain.py` 中：
```py
...
我利用 CC 已经把 muon 接进来了，而且已经在丁峰的基础上完成了 pretrain 的步骤。到现在，我有几个问题：
- 我指定了 muon 去训练，现在预训练已经完成了，那我如何看出 muon 和 adamW 的差异呢？
- 我现在只是对训练有个笼统的概念，需要再细化一些，比如说 pretain, sft 和 rl，这几个步骤，我们需要结合 minimind 的代码去学习一下。并及时知识打结。
...
那么，第一个问题，什么是自回归技术，pretrain 的无监督学习主要的步骤有哪些呢？
...
我们这里要问第二个问题：什么是强化学习？它和监督学习和无监督学习的差别是什么？
...
这里有第三个问题，PPO 是什么的缩写，PPO 和 RM 与 RLHF 的关系是什么？
...
我使用 wandb 会有代理问题，暂时我不用它这个了。我可以同时使用 muon / adamW，各用 2 张卡，然后记录它的打屏信息。
...
5:     $O_t \leftarrow NewtonSchulz5(B_t)$
...
这里的 $B_0$ 意思是存储历史动量（momentum）累积值的张量/矩阵，$\leftarrow$ 是赋值符号，等价于编程里的 `=`，表示把“右边的值赋值给左边”。这里的 $0$ 是全零张量/矩阵，维度和 Muon 要优化的 2D 参数矩阵完全一致。所以这一步的意思是说，把把 $B_0$ 赋值成全 0 矩阵。
...
> 仅对隐藏层的 2D 参数执行此初始化，其他参数按对应优化器（如 AdamW）的规则初始化。
...
- 为什么只是 2D 参数，其他参数不能使用 Muon 呢？

因为它是要正交化

看来 lr 推荐是 0.02，即 2e-2，接下来，我们使用这个去试试
