...
所以，Muon 的设计巧思在于：没有推翻传统的动量优化，而是在其基础上做了一次精准的“补丁式优化” -- 既保留了动量对梯度噪声的平滑作用，又解决了动量带来的方向
单一性的问题，最终实现了“平滑更新 + 丰富方向”的双重效果。
...
> 3、正交化的本质是变换奇异值：文章中通过数学推导证明，NS 迭代对矩阵的处理，最终会转化对奇异值的逐元素多项式映射，调优后的 NS 系数（3.4445, -4.7750, 2.0315） 会让所有
奇异值收敛到 1 附近。最终让矩阵各方向的“信号缩放幅度”趋于均衡。
