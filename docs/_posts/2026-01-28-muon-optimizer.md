...
### pretrain
~~pretrain 是**无监督学习**，解决模型有没有知识的问题。这里的无监督训练的意思是说，它没有 "问题：答案" 这样的配对。是让模型通过自回归技术去预测下一个 token 的方式，把数据集中的知识压缩到模型参数中~~。

准确地来说，pretrain 是自监督学习（Self-Superived Learning），为什么不说是无监督呢？因为它不是没有答案，有答案的，答案就在文本中。对于文本中的一句话，我们先把它切成一个一个的 token，模型读了前 t-1 的 token，做一次前向传播，得到第 t 个 token，然后我们再去和文本中的第 t 个 token 去做对比得到 loss，算梯度，再反向传播更新参数。
...
SFT 是 Supervised Fine-Tuning 的缩写，SFT 是**有监督的**训练。使用的是较低的学习率，因为是微调嘛~
...
强化学习是一种通过与环境交互、试错学习最优策略的机器学习范式。
